---
title: "Appendix S5. The Spatial Individual Specialization Indices applied to different contexts"
...
This scenario comprises individuals which are generalist in their most visited places (i.e. have mean UD values close to each other) and present very low site fidelity. All individuals have UDs with similar SD, which is large compared to the populational UD standard deviation. Below we simulate and illustrate such a population. The black lines represent the UD of the individuals and the red line the population UD.
...
Both the populational (*SpatIS* = 0.22, *SpatICS* = 0.24) and individual values are relatively low (*SpatIS* in the range [0.09, 0.36], *SpatICS* in the range [0.10, 0.40]), as expected. This indicates a generalist population composed of generalist individuals.
...
This scenario comprises individuals which are specialists in their most visited places (i.e. have different mean UD values) but do present a gradient of individual variation in their site fidelity - some individuals have UDs with small SD (high site fidelity), while others have UDs with high SD compared to the populational UD standard deviation (low side fidelity). Below we simulate and illustrate such a population. The black lines represent the UD of the individuals and the red line the population UD.
...
The populational values of the indexes are not so high (*SpatIS* = 0.52, *SpatICS* = 0.58) as the case of the typical specialist population (section 1), but still indicate some degree of specialization in the population. This is clearer when one observes the individual values: there are some individuals that are highly specialized in some areas (maximum *SpatIS* = 0.85, max. *SpatICS* = 0.94), while others are generalists (minimum *SpatIS* = 0.36, min. *SpatICS* = 0.42). This means this population is composed by a mixture of specialist and  generalist individuals, yet is a predominantly specialist population.
...
The populational values of the indexes are relatively low (*SpatIS* = 0.28, *SpatICS* = 0.31), not too above the typical generalist population (section 2), but still indicate some degree of specialization in the population. This is clearer when one observes the individual values: there are individuals that are specialized in some areas (maximum *SpatIS* = 0.61, max. *SpatICS* = 0.68), while others are highly generalists (minimum *SpatIS* = 0.17, min. *SpatICS* = 0.19). This means this population is composed by a mixture of specialist and  generalist individuals, yet is a predominantly generalist population.
...
This examples demonstrate the usefulness of the indices of individual specialization in the use of space in different contexts, from the typical populations composed of only generalist or specilist populations to the more realistic populations composed of specialist and generalist individuals. These examples show that it is important to look beyond the population *SpatIS* and *SpatICS* values to characterize the individual specialization in the use of space within a population. The populational values of these indices may be useful to compare populations, but the distribution of individual *SpatIS* ad *SpatICS* values may provide quality to the assessment of individual specialization, since it is possible to understand how different individual specialization strategies are spread within the population. Plotting individual and populational UDs may also be very important to understand qualitatively how individual specialization in the use of space occurs within populations.

**Pessoal, alguma ideia de como terminar aqui? Ou assim esta ok?**
**O que podemos especular sobre UDs que sigam outras distribuicoes, ou que tenham multiplos locais com alta atividade (distibuicoes multimodais)?**

**Fiquei Pensando se deveriamos incluir um teste em 2d tambem. Poderia fazer algo parecido amostrando pontos de localizacao de distribuicoes Gaussianas bivariadas, variando a media e o SD como aqui, e fazer um teste equivalente... mas daria um pouquinho de trabalho, nao sei se é necessario. O que acham?**