---
title: 'Appendix S3. SpatIS tutorial: calculating the Spatial Individual Specialization Indices'
author: Kerches-Rogeri, P.; Niebuhr, B. B.; Muylaert, R. L., Mello,
  M. A. R. Individual specialization in the use of space by frugivorous bats.
output: 
...
Individuals are naturally different and heterogeneous within populations. These differences may reflect in their physiology, morphology, as well as in their behavior and preferences. To investigate the causes and consequences of interindividual variation, Bolnick et al. [(2003)](http://www.journals.uchicago.edu/doi/abs/10.1086/343878) have related that with niche theory and coined the term "individual specialization". According to this concept, a specialist individual would be one whose niche is much narrower than the niche of the population it is part of. As a consequence, to be considered generalist a population can follow different scenarios: to be composed by generalist individuals with similar niches, by specialists individuals with distinct niches, or by a gradient between this two extremes.

In this context, the niche is generally represented by the type and amount of food items or other resources that are consumed by individuals. As resources are heterogeneously distributed in space in different habitats, it is expected that individual specialization leaves its shadow over space, or even that different environments or geographical space itself may be viewed as resources, so that individuals may be more or less specialists regarding their movement patterns, habitat selection, and use of space.
...
There are two options for calling `SpatIS`. One is to create in the input SpatialPointsDataFrame a "new individual" that represents the whole population (or the rest of the population, for *SpatICS*), with locations of all individuals (or the rest of the individuals, for *SpatICS*) gathered, and pass the "population ID" as one of the arguments when calling `SpatIS`. The other one is to let the function do it for you, if you did not do it yet (in this case, the option `population.ID` should be set to `NULL`, the default). As we still do not have the locations and an ID that represents all the population, we are going to run the second (and easiest) option. We put the results into an object called `observed.SpatIS`.

`SpatIS` function may calculate one or both indexes, *SpatIS* and *SpatICS*. This is defined by the `index` parameter. The method to assess the overlap between UDs or areas of use is defined by the `method` parameter. Other arguments for the `kernelUD` and `kerneloverlap` function are also accepted.
...
3) `SpatIS.individual`: the value of the Spatial Individual Specialization index for each individual (i.e., the level of overlap between their utilization distribution and the population's utilization distribution, a value from 0 to 1):
...
4) `SpatIS.population`: the value of the Spatial Individual Specialization index for the whole population, defined as the individual SpatIS averaged over all individuals:
...
6) `SpatICS.population`: the value of the Spatial Individual Complementary Specialization index for the whole population, defined as the SpatICS averaged over individuals:
...
# all individuals that are the ID = 1 are considered as rest
