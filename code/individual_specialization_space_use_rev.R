...
for(i in 1:length(ids)){  ## loop for individuals
  ...
  for(k in 5:length(temp)){    			## loop for sample size from 5 locations to all locations
    ...
    cumulative[k] <- kernel.area(kde, percent=95)
  }  
...
for(i in 1:length(homeranges)) {
  # rgdal::writeOGR(homeranges[[i]], "output", paste0("kde50_", as.character(homeranges[[i]]$id)),
...
randomized <- SpatIS_randomize(calculated.spatis, iterations = 99)
